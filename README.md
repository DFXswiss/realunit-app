...
| `handbook-deploy.yaml`       | Push `staging` · manual                                       | Builds the handbook image from `staging` and deploys it to PRD via the reusable `handbook.yaml` (`:latest`, handbook.realunit.app). No DEV environment anymore — the former DEV instance is retired; `handbook-build-check.yaml` is the pre-merge safety net instead of an in-run "DEV green before PRD" check |
