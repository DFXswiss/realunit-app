...
**Touched UI 100 % rule.** Every PR-touched file under `lib/screens/**` or `lib/widgets/**` (pages, views, widgets — not cubit/bloc folders, not `*.g.dart`) must reach 100 % line coverage, measured per file. CI runs `scripts/check-touched-ui-coverage.sh coverage/lcov.info origin/<base-ref>` **after** `flutter test --coverage` and **before** `lcov --extract`, against the full tracefile so widget lines are still present. The packages/cubits/blocs floor is **not** widened to the whole widget tree — only files the PR actually changes are gated. A neglected UI file that is touched must be brought fully to 100 % in the same PR.
